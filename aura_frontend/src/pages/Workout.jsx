...
const CountdownTimer = ({ exercise, onComplete }) => {
  ...
  return (
    ...
    >
      ...
      {isDone && (
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-sm text-[#4A6B5D] font-medium">
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-background rounded-[0.875rem] p-3.5 flex flex-col gap-1.5 border border-border">
          ...
          <span className="text-lg font-light text-[#4A6B5D] font-medium">+{exercise.estimated_xp}</span>
        </div>
      </div>
...
const TrackList = ({ modalityKey, onBack, onSelectTrack }) => {
  ...
  return (
    <motion.div key="level-2" variants={pageVariants} initial="initial" animate="animate" exit="exit">
      ...
      <SectionHeader title={modality.title} subtitle="Choose a training program to explore its structured sessions." />

      <motion.div variants={cardStagger} initial="initial" animate="animate" className="grid grid-cols-1 md:grid-cols-2 gap-4">
...
const SubSectionGrid = ({ track, onBack, onSelectSubSection }) => {
  return (
    <motion.div key="level-3" variants={pageVariants} initial="initial" animate="animate" exit="exit">
      ...
      <SectionHeader title={track.title} subtitle="Select a focus session to view its complete exercise roster." />

      <motion.div variants={cardStagger} initial="initial" animate="animate" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {isDone && (
                        <button
                          onClick={(e) => { e.stopPropagation(); resetExerciseCompletion(exercise.id, exercise.estimated_xp); }}
                          title="Reset completion" className="opacity-40 hover:opacity-80 transition-opacity w-6 h-6 flex items-center justify-center rounded-full hover:bg-[#4A6B5D]/10"
                        >
          ...
          <AnimatePresence mode="wait">
            {selectedExercise ? (
              <div key={selectedExercise.id}>
                <ExercisePreviewPane exercise={selectedExercise} onClose={() => setSelectedExercise(null)} onBeginSession={handleBeginSession} />
                <AnimatePresence>
...
const Workout = () => {
  ...
  const handleSelectModality = (key) => { setSelectedModalityKey(key); setLevel(2); };
  const handleSelectTrack = (track) => { setSelectedTrack(track); setLevel(3); };
  const handleSelectSubSection = (sub) => { setSelectedSubSection(sub); setLevel(4); };
    ...
    <div className="min-h-screen bg-background transition-colors duration-300">
      <div className="max-w-6xl mx-auto px-5 py-10 md:px-10 md:py-14 pb-10">
        <AnimatePresence mode="wait">
          ...
          {level === 3 && selectedTrack && <SubSectionGrid key="l3" track={selectedTrack} onBack={handleBack} onSelectSubSection={handleSelectSubSection} />}
          {level === 4 && selectedSubSection && <ExerciseRoster key="l4" subSection={selectedSubSection} onBack={handleBack} completeWorkoutAction={handleCompleteWorkout} />}
        </AnimatePresence>
      </div>
    </div>
  );
